...
class Ion_auth {
     ...
     * @author Mathew
     * */
    public function register($identity, $password, $email, $nombre, $group_ids = array()) { //need to test email activation
        $this->ion_auth_model->trigger_events('pre_account_creation');

        $email_activation = $this->config->item('email_activation', 'ion_auth');

        $id = $this->ion_auth_model->register($identity, $password, $email, $nombre, $group_ids);

        if (!$email_activation) {
            if ($id !== FALSE) {                
                $this->set_message('account_creation_successful');
               // $this->session->set_flasdata('message','Usuario registrado');
